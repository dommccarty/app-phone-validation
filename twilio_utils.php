...
function send_sms($from_number, $to_number, $sms_body) {
	...
	if (!is_string($sms_body)) return null;
	
	if (strlen($from_number) == 10) $from_number = "+1" . $from_number;
...
function send_mms($from_number, $to_number, $text, $image_urls) { //last is a numeric array
...
function giveTwilioFail($sid) { //non-null means yes.
